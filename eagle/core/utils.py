...
def balance_resource(task, real_used, balance=[0.8, 0.2]):
    """
    获取balance后的资源使用率
    :returns: 使用的CPU, 内存
    :rtype: int
    """
    used_cpu, used_mem = None, None
    if task and real_used:
        used_cpu = float(float(task['need_cpu']) * balance[0] +
                         float(float(real_used['cpu']) * balance[1] / 100000))
        used_mem = float(float(task['need_mem']) * balance[0] +
                         float(real_used['memory']) / 1000 * balance[1])
...
def bool_from_string(subject, strict=False, default=False):
    ...
    if lowered in TRUE_STRINGS:
        ...
        acceptable = ', '.join("'%s'" % s for s in sorted(
