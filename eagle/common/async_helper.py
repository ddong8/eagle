...
class CallbackController(object):
    ...
    def _check_auth(self, req, resp):
        strict_client = settings.CELERY_WORKER.get(
            'worker.callback.strict_client', True)
        global_allow_hosts = settings.CELERY_WORKER.get(
            'worker.callback.allow_hosts', None)
        name_allow_hosts = settings.CELERY_WORKER.get(
            'worker.callback.name', None)
        # 防止controller的name中带有.特殊字符，无法取出值
        if name_allow_hosts is not None:
            name_allow_hosts = name_allow_hosts.get(
                self.name, {}).get('allow_hosts', None)
...
def send_callback(url_base, func, data, request_context=None, **kwargs):
    ...
    vars, pattern = uri_util.compile_uri_template(url)
    for var in vars:
        url = url.replace('{%s}' % var, str(kwargs.get(var)))
    url_base = settings.PUBLIC_ENDPOINT if url_base is None else url_base
    url = url_base + url
    http_method = getattr(http_util.RestfulJson, method, None)
    LOG.debug('######## async_helper:send_callback %s %s, data: %s',
              method, url, data)
        ...
        class __wraps_c(object):
            ...
            def context(self, **kwargs):
                new_instance = self.__class__(
                    base_url=self.__base_url, requests_ctx=kwargs)
                return new_instance

            def baseurl(self, url_prefix):
                new_instance = self.__class__(
                    base_url=url_prefix, requests_ctx=self.__requests_ctx)
                return new_instance
